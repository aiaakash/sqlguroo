...
import CloseAIExportButtons from './CloseAIExportButtons';
import SqlEditorReference, { hasSqlEditorReference } from './Parts/SqlEditorReference';
...
const DisplayMessage = ({ text, isCreatedByUser, message, showCursor }: TDisplayProps) => {
  ...
  return (
    <Container message={message}>
      {/* Export buttons for any markdown tables in responses */}
      {!isCreatedByUser && <CloseAIExportButtons text={text} />}
      <div
        className={cn(
          'markdown prose message-content dark:prose-invert light w-full break-words',
          isSubmitting && 'submitting',
          showCursorState && text.length > 0 && 'result-streaming',
          isCreatedByUser && !enableUserMsgMarkdown && !hasSqlRef && 'whitespace-pre-wrap',
