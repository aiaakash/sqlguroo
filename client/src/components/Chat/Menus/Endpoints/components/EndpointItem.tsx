...
import { CheckCircle2, MousePointerClick, SettingsIcon, Database } from 'lucide-react';
...
export function EndpointItem({ endpoint, endpointIndex }: EndpointItemProps) {
  ...
  // Check if this is the closeAI endpoint with analytics connections
  const isCloseAIWithConnections = endpoint.value === EModelEndpoint.closeAI && 
    endpoint.connectionNames && 
    Object.keys(endpoint.connectionNames).length > 0;
  ...
  // For closeAI endpoint with analytics connections, render connections as flat items
  if (isCloseAIWithConnections && endpoint.models) {
    return (
      <>
        {/* Render connection models as flat items */}
        {endpoint.models.map((model) => {
          const connectionId = model.name;
          const connectionName = endpoint.connectionNames?.[connectionId] || connectionId;
          const connectionType = endpoint.connectionTypes?.[connectionId] || '';
          
          return (
            <MenuItem
              key={connectionId}
              id={`endpoint-${endpoint.value}-connection-${connectionId}`}
              name="model"
              value={connectionId}
              onClick={() => handleSelectModel(endpoint, connectionId)}
              className="group flex w-full cursor-pointer items-center justify-between gap-1.5 py-1.5 text-xs"
            >
              <div className="flex min-w-0 items-center gap-2">
                <Database className="size-3.5 shrink-0 text-text-secondary" aria-hidden="true" />
                <div className="flex flex-col">
                  <span className="truncate text-left text-text-primary">{connectionName}</span>
                  {connectionType && (
                    <span className="text-[9px] text-text-tertiary uppercase">{connectionType}</span>
                  )}
                </div>
              </div>
            </MenuItem>
