...
export function ModelSelectorProvider({ children, startupConfig }: ModelSelectorProviderProps) {
  ...
  // Fetch Analytics connections for model selector
  // Note: We pass empty string to fetch user's own connections
  // The backend now properly filters by user, so this returns only connections owned by the current user
  const { data: analyticsConnections = [] } = useAnalyticsConnections('');
