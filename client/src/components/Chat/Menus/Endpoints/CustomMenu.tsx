    ...
    <Ariakit.MenuProvider store={menuStore} values={values} setValues={onValuesChange}>
      ...
      <Ariakit.Menu
        ...
        className={cn(
          ...
          'w-[var(--menu-width,auto)] min-w-[260px] flex-col overflow-auto rounded-lg border border-border-light',
          'bg-presentation px-2 py-1.5 text-xs text-text-primary shadow-lg',
          'max-w-[calc(100vw-4rem)] sm:max-h-[50vh] sm:max-w-[320px]',
          searchable && 'p-0',
        )}
      >
              ...
              <div className="sticky top-0 z-10 bg-inherit p-1">
                <div className="relative">
                  <Ariakit.Combobox
                    ...
                    render={combobox}
                    className={cn(
                      'peer flex h-7 w-full items-center justify-center rounded-md border-none bg-transparent px-2 text-sm',
                      'sm:h-6 sm:text-xs',
                      'focus:outline-none focus:ring-0 focus-visible:ring-2 focus-visible:ring-primary',
                    )}
                  />
                  {comboboxLabel && (
                    <label className="pointer-events-none absolute left-2.5 top-2 text-xs text-text-secondary transition-all duration-200 peer-[:not(:placeholder-shown)]:-top-1 peer-[:not(:placeholder-shown)]:left-1.5 peer-[:not(:placeholder-shown)]:bg-presentation peer-[:not(:placeholder-shown)]:text-[10px] sm:top-1">
                      {comboboxLabel}
                    </label>
                  )}
                </div>
...
export const CustomMenuItem = React.forwardRef<HTMLDivElement, CustomMenuItemProps>(
  function CustomMenuItem({ name, value, ...props }, ref) {
    ...
    const defaultProps: CustomMenuItemProps = {
      ...
      ...props,
      className: cn(
        'relative flex cursor-default items-center gap-2 rounded-md px-2 py-0.5 outline-none! scroll-m-1 scroll-mt-[calc(var(--combobox-height,0px)+var(--label-height,4px))] aria-disabled:opacity-25 data-[active-item]:bg-black/[0.075] data-[active-item]:text-black dark:data-[active-item]:bg-white/10 dark:data-[active-item]:text-white text-xs min-w-0 w-full before:absolute before:left-0 before:top-0.5 before:bottom-0.5 before:w-0.5 before:bg-transparent before:rounded-full data-[active-item]:before:bg-black dark:data-[active-item]:before:bg-white',
        props.className,
      ),
    };
