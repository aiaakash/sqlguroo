...
import { SqlExecutorPanel } from '~/components/Chat/SqlExecutor';
...
export default function Presentation({ children }: { children: React.ReactNode }) {
  ...
  const sqlExecutorVisibility = useRecoilValue(store.sqlExecutorVisibility);
  ...
  /**
   * Memoize SQL executor JSX to prevent recreating it on every render
   */
  const sqlExecutorElement = useMemo(() => {
    if (sqlExecutorVisibility === true) {
      return <SqlExecutorPanel />;
    }
    return null;
    ...
    <DragDropWrapper className="relative flex w-full grow overflow-hidden bg-presentation">
      <SidePanelProvider>
        <SidePanelGroup
          ...
          sqlExecutor={sqlExecutorElement}
        >
