...
const CodeBar: React.FC<CodeBarProps & { isSql?: boolean; messageId?: string; conversationId?: string; codeContent?: string }> = React.memo(
  ({ lang, error, codeRef, blockIndex, plugin = null, allowExecution = true, isSql, messageId, conversationId, codeContent }) => {
...
const CodeBlock: React.FC<CodeBlockProps> = ({
  ...
  const { messageId, partIndex, conversationId } = useMessageContext();
  ...
  // Extract code content from codeChildren on mount
  useEffect(() => {
    if (codeChildren) {
      ...
      setCodeContent(extractText(codeChildren));
    }
  }, [codeChildren]);
  ...
  // Detect SQL language
  const isSql = !isNonCode && /^(sql|mysql|postgresql|postgres|sqlite|mssql|oracle)$/i.test(lang);
