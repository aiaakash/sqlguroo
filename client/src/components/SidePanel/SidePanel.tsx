...
const SidePanel = ({
  ...
  }, [isCollapsed, panelRef, setMinSize, setIsCollapsed, setFullCollapse, setCollapsedSize, navCollapsedSize]);
  ...
  return (
    <>
      ...
      <ResizablePanel
        tagName="nav"
        id="controls-nav"
        order={hasArtifacts || hasSqlExecutor ? (hasArtifacts && hasSqlExecutor ? 4 : 3) : 2}
        ...
        onExpand={() => {
          // Prevent expansion when the panel is supposed to be collapsed
          // This prevents auto-expansion when other panels (SQL executor, artifacts) are added
          if (isCollapsed) {
            return;
          }
          setIsCollapsed(false);
        ...
        className={cn(
          'sidenav hide-scrollbar border-l border-border-light bg-sidebar py-1 transition-opacity',
