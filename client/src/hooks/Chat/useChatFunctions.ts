...
export default function useChatFunctions({
  ...
  const analyticsModel = useRecoilValue(store.analyticsModel);
  const agentType = useRecoilValue(store.agentType);
  const sqlEditorLock = useRecoilValue(store.sqlEditorLock);
  const sqlEditorContent = useRecoilValue(store.sqlEditorContent);
  ...
  const ask: TAskFunction = (
    ...
    // If SQL Editor Lock is enabled, include the editor's SQL content with the message
    // This allows the AI to reference the current SQL when generating responses
    let finalText = text;
    if (sqlEditorLock && sqlEditorContent?.trim()) {
      finalText = `${text}\n\n[Current SQL in Editor]:\n\`\`\`sql\n${sqlEditorContent.trim()}\n\`\`\``;
    }
    ...
    // Add analytics model and agent type for analytics/closeAI endpoints
    if (isAnalyticsEndpoint(endpoint) || endpoint === EModelEndpoint.closeAI) {
      const finalAnalyticsModel = analyticsModel || 'z-ai/glm-4.5-air:free';
      const finalAgentType = agentType || 'react';
      (endpointOption as any).analyticsModel = finalAnalyticsModel;
      (endpointOption as any).agentType = finalAgentType;
      console.log('[useChatFunctions] Added analyticsModel and agentType to endpointOption:', { 
        endpoint, 
        analyticsModelFromRecoil: analyticsModel, 
        finalAnalyticsModel,
        agentTypeFromRecoil: agentType,
        finalAgentType,
        endpointOption: { ...endpointOption, analyticsModel: finalAnalyticsModel, agentType: finalAgentType }
      }); // ⭐ Debug: Model and agent type added to endpointOption
    }
