...
export default function useStepHandler({
  ...
  const updateContent = (
    ...
    if (
      ...
      const incomingThink = contentPart.think;
      const hasOpeningTag = incomingThink.includes(' ');
      const hasClosingTag = incomingThink.includes(' ');
      const isCompleteThinkingMessage = hasOpeningTag && hasClosingTag;

      const update: ReasoningDeltaUpdate = {
        type: ContentTypes.THINK,
        think: isCompleteThinkingMessage
          ? incomingThink
          : (currentContent.think || '') + incomingThink,
      };
