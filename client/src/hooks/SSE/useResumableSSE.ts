...
export default function useResumableSSE(
  ...
  const subscribeToStream = useCallback(
    (currentStreamId: string, currentSubmission: TSubmission, isResume = false) => {
      ...
      // Determine stream endpoint based on conversation endpoint
      const endpoint = currentSubmission.conversation?.endpoint;
      const streamBasePath = endpoint === EModelEndpoint.analytics
        ? '/api/analytics/chat/stream'
        : '/api/agents/chat/stream';

      const baseUrl = `${streamBasePath}/${encodeURIComponent(currentStreamId)}`;
      const url = isResume ? `${baseUrl}?resume=true` : baseUrl;
      console.log('[ResumableSSE] Subscribing to stream:', url, { isResume, endpoint });
