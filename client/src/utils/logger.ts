...
// Tags to exclude from logging (reduce noise in console)
const excludedTags = ['message_scrolling', 'tools', 'icon_data', 'MessageAudio'];
...
function shouldLog(tag: string): boolean {
  ...
  if (tag && excludedTags.some((excluded) => tag.toLowerCase().includes(excluded.toLowerCase()))) {
    return false;
  }
