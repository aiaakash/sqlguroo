...
export const providerConfigMap: Record<string, InitializeFn> = {
  ...
  [EModelEndpoint.closeAI]: initializeOpenAI,
...
export function getProviderConfig({
  ...
  if (!getOptions) {
    // Handle closeAI specifically (can come as 'closeAI' or 'closeai')
    if (provider === 'closeAI' || provider === 'closeai' || provider === EModelEndpoint.closeAI) {
      const closeAIOptions = providerConfigMap[EModelEndpoint.closeAI];
      if (closeAIOptions != null) {
        getOptions = closeAIOptions;
  ...
  if (!getOptions) {
    ...
    if (knownEndpoints[providerLower]) {
      ...
      const foundOptions = providerConfigMap[enumValue];
