...
export async function initializeAgent(
  ...
  // ⭐ Debug: Log that initializeAgent is being called
  console.log('[initializeAgent] Function called:', {
    endpoint: endpointOption?.endpoint || agent?.endpoint,
    isInitialAgent,
    endpointOptionKeys: endpointOption ? Object.keys(endpointOption) : 'NO endpointOption',
    analyticsModelInEndpointOption: endpointOption?.analyticsModel || 'NOT IN endpointOption',
    agentTypeInEndpointOption: endpointOption?.agentType || 'NOT IN endpointOption',
  });
  ...
  // ⭐ For CloseAI endpoint, extract analyticsModel and agentType from endpointOption and add to model_parameters
  // This allows initializeCloseAI to receive them and pass via custom header
  const finalModelOptions = {
    ...modelOptions,
    model: agent.model,
  };

  // ⭐ Debug: Log all cases for CloseAI endpoint
  if (isInitialAgent && endpointOption?.endpoint === EModelEndpoint.closeAI) {
    console.log('[initializeAgent] CloseAI endpoint detected:', {
      endpoint: endpointOption.endpoint,
      endpointOptionKeys: Object.keys(endpointOption || {}),
      analyticsModelInEndpointOption: endpointOption?.analyticsModel || 'NOT IN endpointOption',
      agentTypeInEndpointOption: endpointOption?.agentType || 'NOT IN endpointOption',
      hasAnalyticsModel: !!endpointOption?.analyticsModel,
      hasAgentType: !!endpointOption?.agentType,
    });
  }

  // If this is the initial agent and endpoint is closeAI, pass analyticsModel and agentType through model_parameters
  if (isInitialAgent && endpointOption?.endpoint === EModelEndpoint.closeAI) {
    if (endpointOption?.analyticsModel) {
      (finalModelOptions as Record<string, unknown>).analyticsModel = endpointOption.analyticsModel;
      console.log('[initializeAgent] Added analyticsModel to finalModelOptions:', {
        endpoint: endpointOption.endpoint,
        analyticsModel: endpointOption.analyticsModel,
      });
    }
    if (endpointOption?.agentType) {
      (finalModelOptions as Record<string, unknown>).agentType = endpointOption.agentType;
      console.log('[initializeAgent] Added agentType to finalModelOptions:', {
        endpoint: endpointOption.endpoint,
        agentType: endpointOption.agentType,
      });
    }
    console.log('[initializeAgent] finalModelOptions keys:', Object.keys(finalModelOptions));
  } else if (isInitialAgent && endpointOption?.endpoint === EModelEndpoint.closeAI) {
    console.log('[initializeAgent] CloseAI endpoint but analyticsModel/agentType NOT FOUND:', {
      endpoint: endpointOption.endpoint,
      endpointOptionKeys: Object.keys(endpointOption || {}),
      hasAnalyticsModel: !!endpointOption?.analyticsModel,
      hasAgentType: !!endpointOption?.agentType,
