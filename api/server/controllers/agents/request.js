...
const ResumableAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    // Early quota check to provide immediate feedback
    const { exceeded, limit } = await subscriptionService.checkQueryLimit(userId);
    if (exceeded) {
      const limitError = `You have reached your query limit of ${limit} queries for your current plan. Please upgrade your plan to continue.`;
      logger.warn(`[AgentController] User ${userId} exceeded query limit: ${limit}`);
      return res.status(429).json({ text: limitError });
    }
    ...
    return; // Ensure we don't proceed or call next() if we've already handled the response
  }
};
